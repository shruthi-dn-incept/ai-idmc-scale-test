...
# ── phase 1: extract ────────────────────────────────────────────────────────
def extract():
    out = _sh(f"{sys.executable} -u extract_cdgc_columns.py --workers 16")
...
# ── phase 2: taxonomy ───────────────────────────────────────────────────────
def taxonomy():
    _sh(f"{sys.executable} -u build_taxonomy_full.py")
...
def colterm():
    ...
    json.dump(m, open("colterm_map.json", "w"), indent=1)
...
def domain():
    ...
    for name in terms:
        ...
        if tid:
            termids[name] = tid
    json.dump(termids, open("term_ids.json", "w"), indent=1)
...
def system_ds():
    ...
    json.dump(out, open("system_dataset.json", "w"), indent=1)
    return {"system": 1, "datasets": len([v for v in out.values() if v])}


# ── phase 6: generate DQRO file ─────────────────────────────────────────────
def gen_dqro():
    out = _sh(f"{sys.executable} -u generate_dqro_import.py --origin-filter {ORIGIN} "
...
def import_dqro():
    import cdgc_bulk_import as imp
...
# ── phase 8: curate ─────────────────────────────────────────────────────────
def curate():
    out = _sh(f"{sys.executable} -u curate_scale.py --batch 50 --workers 14")
...
def scan():
    ...
    json.dump(jobs, open("mcc_scan_jobs.json", "w"), indent=1)
    return {"scans": jobs}
...
ALL = [("cleanup", None), ("extract", extract), ("taxonomy", taxonomy), ("colterm", colterm),
       ("domain", domain), ("system_ds", system_ds), ("gen_dqro", gen_dqro),
