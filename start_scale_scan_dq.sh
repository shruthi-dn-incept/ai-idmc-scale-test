#!/bin/bash
# ACA Job entrypoint: start MCP servers, then run the scan + DQ-asset scale runner.
# Arg $1 = table limit (a number), or "all" / empty for the full catalog.
set -e

echo "=== Starting governance_engine_mcp (:9765) ==="
python governance_engine_mcp.py &
echo "=== Starting ai_governance_mcp (:9770) ==="
python ai_governance_mcp.py &

echo "=== Waiting for MCP servers ==="
for PORT in 9765 9770; do
  for i in $(seq 1 40); do
    if python -c "import socket; s=socket.create_connection(('127.0.0.1',$PORT),1); s.close()" 2>/dev/null; then
      echo "  Port $PORT ready"; break
    fi
    sleep 2
  done
done

LIMIT="${1:-all}"
echo "=== Running scale_scan_dq (limit=$LIMIT) ==="
if [ "$LIMIT" = "all" ]; then
  python scale_scan_dq.py --all
else
  python scale_scan_dq.py --limit "$LIMIT"
fi
echo "=== Done ==="
